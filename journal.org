...
#+HEADER: :var rs_search_space_2="supersim_experiments/results/weighted_injection_time_saturation/rs_200_samples_1_iterations_search_space_2.csv"
...
rs_search_space_df <- bind_rows(read.csv(rs_search_space, header = TRUE),
                                read.csv(rs_search_space_2, header = TRUE))
...
rs_search_space_df <- rs_search_space_df %>%
  filter(injection_rate_1 > 0.02 & injection_rate_2 > 0.02) %>%
  filter(is.finite(performance_metric))
...
p1 <- ggplot() +
  ...
  geom_point(data = filter(rs_search_space_df,
                           performance_metric == min(performance_metric)),
             size = 4,
             color = "red",
             shape = 2,
             stroke = 2,
...
output <- optFederov(data = rs_search_space_df,
                     performance_metric ~ injection_rate_1 +
                       injection_rate_2 +
...
#                  tau = 0.05,
#                  formula = performance_metric ~ injection_rate_1 +
#                    injection_rate_2 +
#                    I(1 / injection_rate_1) +
#                    I(1 / injection_rate_2) +
#                    I(injection_rate_2 ^ 2) +
